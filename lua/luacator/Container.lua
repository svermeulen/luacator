...
do
  ...
  local _base_0 = {
    bind = function(self, ...)
      Assert.that(not self.startedBinding)
    ...
    _resolveInternal = function(self, identifier, matchMany, asFactory)
      Assert.that(not Util.contains(self.identifierLookupsInProgress, identifier), "Found circular dependency!  Object graph: " .. tostring(self:_getObjectGraph()) .. " -> " .. tostring(identifier))
      ...
      return Util.try({
        ["do"] = function()
          ...
          Assert.that(providers, "Could not find dependency with identifier '" .. tostring(identifier) .. "'")
          Assert.that(#providers == 1, "Found multiple providers when only one was expected for identifier '" .. tostring(identifier) .. "'")
